...
void CPlot::AddPlotItem(CPlotItem item)
{
  fItems.push_back(item);
}

void CPlot::AddPlotItem(CPlotItem item, TString label, TString legopt)
{
  fItems.push_back(item);
  ...
  if     (item.hist1D) { fLeg->AddEntry(item.hist1D,label,legopt); }
  else if(item.hist2D) { fLeg->AddEntry(item.hist2D,label,legopt); }
  else if(item.graph)  { fLeg->AddEntry(item.graph, label,legopt); }
}


//--------------------------------------------------------------------------------------------------
void CPlot::AddHist1D(TH1D *h, TString drawopt, int color, int linesty, int fillsty, int linewidth)
{
  if(!h)
    return;
  ...
  assert(h->GetLineWidth()==2);

  if(drawopt.CompareTo("E2",TString::kIgnoreCase)==0) {
    h->SetMarkerSize(0);
  } else if(drawopt.Contains("E",TString::kIgnoreCase)) {
    if(h->GetNbinsX()>20) { h->SetMarkerSize(0.9); }
    else                  { h->SetMarkerSize(1.2); }
  }
...
void CPlot::AddHist1D(TH1D *h, TString label, TString drawopt, int color, int linesty, int fillsty, int linewidth)
{
  if(!h)
    return;
  ...
  AddHist1D(h,drawopt,color,linesty,fillsty,linewidth);
}

void CPlot::AddHist1D(TFile *f, TString histName, TString drawopt, int color, int linesty, int fillsty, int linewidth)
{
  if(!f)
    return;
...
void CPlot::AddHist1D(TFile *f, TString histName, TString label, TString drawopt, int color, int linesty, int fillsty, int linewidth)
{
  if(!f)
    return;
  
  TH1D *h = (TH1D*)f->FindObjectAny(histName);
  AddHist1D(h,label,drawopt,color,linesty,fillsty,linewidth);
}
...
void CPlot::AddGraph(TGraph *gr, TString label, TString drawopt, int color, int marksty, int linesty)
{
  ...
  } else if(drawopt.Contains("2") || drawopt.Contains("3") || drawopt.Contains("4") || drawopt.Contains("5")) {
...
void CPlot::AddFcn2D(TF2* fcn, int color, int linesty)
{
  if(!fcn)
    return;
  ...
  fcn->SetLineColor(color);
  fcn->SetLineStyle(linesty);
  fFcns2D.push_back(fcn);
}
/*
void CPlot::AddFcn2D(TF2* fcn, TString label, int color, int linesty)
{
  if(!fcn)
    return;
...
void CPlot::Draw(TCanvas *c, bool doSave, TString format, Int_t subpad)
{
  ...
  if(!fFcns.size() && fFcns2D.size() && !fLines.size() && !fBoxes.size() && !fTextBoxes.size() && !fItems.size() && !fRooPlot)
    return;
...
// gr->GetXaxis()->SetRangeUser(fXmin,fXmax);
    ...
    if(vHists.size()==0) {
      ...
      //MT
      vGraphs[0]->GetXaxis()->SetTitleOffset(0.9);
      vGraphs[0]->GetYaxis()->SetTitleOffset(0.9);
      //
  ...
  for(uint i=0; i<fFcns2D.size(); i++)
    (i==0 && vHists.size()==0 && vGraphs.size()==0) ? fFcns2D[i]->Draw() : fFcns2D[i]->Draw("sameC");
